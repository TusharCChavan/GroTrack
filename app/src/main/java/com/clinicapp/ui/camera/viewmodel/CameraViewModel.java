...
public class CameraViewModel extends BaseViewModel {
    ...
    public void onNext(long analysisID,long patientID) {
        final CameraCaptureState state = captureStateLiveData.getValue();
        Log.e("path--",state.getPath());
        File file1 = new File(state.getPath());
        Log.e("path--1",file1.exists()+"");
        File file = new File(state.getPath().replace("file://",""));
        final CameraPositions position = positions.get(currentPosition);
        Log.e( "onNext: ",file.getPath()+" - "+file.exists() );
        ...
        Log.e( "onNext: 1",imagePart.body()+"" );
        uploadImage(mainType, subType, imagePart,analysisID,patientID);
    }


    private void uploadImage(Part mainType, Part subType, Part image,long analysisID,long patientID) {
        captureCompleteLiveData.setValue(AsyncResponse.loading());
        repository.api.uploadImage(mainType,subType,image,analysisID,patientID)
